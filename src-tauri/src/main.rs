...
fn generate_aws_config(state: State<AppState>) -> Result<String, String> {
    ...
    config.push_str("# Usage: aws s3 ls --profile <profile-name>\n");
    config.push_str("# Refresh tokens: Use AWS Login Hub app -> Refresh SSO Tokens\n\n");
    ...
    Ok(format!("Generated {} profiles at {}. Available system-wide in any terminal.", clients.len(), config_path.display()))
}
