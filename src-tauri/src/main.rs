...
fn generate_aws_config(state: State<AppState>) -> Result<String, String> {
    ...
    config.push_str("# Do not edit manually. Regenerate from the app.\n\n");
    ...
    for client in &clients {
        ...
        config.push_str(&format!("sso_region = us-east-1\n"));
        config.push_str(&format!("sso_account_id = \n"));
        config.push_str(&format!("sso_role_name = \n"));
        config.push_str(&format!("region = us-east-1\n"));
        config.push_str(&format!("output = json\n"));
        config.push_str("\n");
    }
    ...
    // Read existing config and append/replace
    let existing = std::fs::read_to_string(&config_path).unwrap_or_default();
    let marker = "# AWS SSO Profiles - Auto-generated by AWS Login Hub";

    let final_config = if existing.contains(marker) {
        // Replace the auto-generated section
        let parts: Vec<&str> = existing.splitn(2, marker).collect();
        format!("{}{}", parts[0].trim_end(), if parts[0].is_empty() { "".to_string() } else { "\n\n".to_string() }) + &config
    ...
    Ok(format!("Generated {} profiles at {}", clients.len(), config_path.display()))
}
